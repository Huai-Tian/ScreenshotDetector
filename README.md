...
- `AccessibilityService` (Requires enabling this app's accessibility service): `TYPE_WINDOW_STATE_CHANGED` events feed audio-recording source suspicion (a candidate source that works without Usage access); `getWindows()` is the only app-layer cross-app window enumeration — the currently focused external window is the freeform-window attribution *fact* (preferred over the usage-stats inference), and the highest-layer overlapping external window is the occlusion source of the incomplete-presentation card. Only window package names and window properties are read; window content is never read
