...
- **Media library monitoring**  
  Monitors media library changes via `ContentObserver`: the image store identifies newly added screenshots, and the video store identifies newly added recordings by screen-recording naming features (screenrecord / 屏幕录制, covering AOSP and Chinese ROM conventions); scans all external storage volumes (SD-card volumes included, API 30+) and covers the `Download/` directory (videos there do not enter the Video collection; included after MIME filtering); a 15-second lookback query runs at registration, covering events right before the app was opened. The card carries the writer's attribution (`owner_package_name` hidden column; cross-app visibility varies by ROM and degrades gracefully; a SystemUI writer is annotated as a system key-press screenshot — a cross-version fallback for when this app is in the background or on Android 11–13 where the key-press callback has no signal)

- **Shell / ADB screenshot detection**  
  Detects screenshots produced through the adb shell channel: an image-library row whose writer attribution is `com.android.shell` (the uid-attributed package of adb shell) is a factual record of a shell-channel capture (e.g. `adb shell screencap`) — the write location and file name are unconstrained, so the scan is not limited to screenshot naming features (including the `Download/` directory and non-primary volumes). Key-press screenshots are written by SystemUI and third-party apps by their own package names, so the writer identity alone distinguishes the channel; the card shows the file name

- **Shell / ADB screen recording detection**  
  Detects recordings produced through the adb shell channel: a video-library row (Downloads collection included) whose writer attribution is `com.android.shell` (e.g. `adb shell screenrecord`) is reported as a separate card (with the file name) — mirroring the screenshot side: write location and name are unconstrained by feature words, so command-line recordings with custom file names are not missed
...
- `ContentObserver` + `MediaStore`: Detects newly added screenshots (image store) / recordings (video store, matched by screen-recording naming features); queries all external volumes (`getExternalVolumeNames`, API 30+) plus the Downloads collection (`Download/` directory, MIME-filtered); the query Bundle carries `MATCH_PENDING` to include pending rows at write time for an earlier detection, and the projection includes the `owner_package_name` hidden column for writer attribution. Rows attributed to `com.android.shell` are reported as separate Shell / ADB screenshot / recording cards (the scan is not limited to naming features, since shell writes are unconstrained in location and name); a SystemUI writer is annotated as a system key-press screenshot
- `FileObserver`: Detects file creation/move in the screenshot/recording directories (`Pictures/Screenshots`, `DCIM/Screenshots`, `Movies/ScreenRecords`, `DCIM/ScreenRecorder`, `Movies` — multiple directories)
